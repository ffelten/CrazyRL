...
class Surround(BaseParallelEnv):
    ...
    def _compute_obs(self):
        ...
        for agent in self._agents_names:
            ...
            obs[agent] = jnp.append(obs[agent], self._target_location["unique"])

            for other_agent in self._agents_names:
                if other_agent != agent:
                    obs[agent] = jnp.append(obs[agent], self._agent_location[other_agent])
    ...
    def _compute_action(self, actions):
        ...
        for agent in self.agents:
            ...
            target_point_action[agent] = jnp.clip(
    ...
    def _compute_reward(self):
        ...
        for agent in self._agents_names:
            ...
            for other_agent in self._agents_names:
                if other_agent != agent:
                    reward[agent] += jnp.linalg.norm(self._agent_location[agent] - self._agent_location[other_agent])
            ...
            reward[agent] += 0.95 * (
                2 * self.size - jnp.linalg.norm(self._agent_location[agent] - self._target_location["unique"])
            )
            ...
            if jnp.linalg.norm(self._agent_location[agent] - self._target_location["unique"]) < 0.2:
                reward[agent] -= 100
    ...
    def _compute_terminated(self):
        ...
        for agent in self.agents:
            ...
            # collision with the target
            terminated[agent] = terminated[agent] or (
                jnp.linalg.norm(self._agent_location[agent] - self._target_location["unique"]) < 0.2
            )

            if terminated[agent]:
                for other_agent in self.agents:
                    terminated[other_agent] = True
                self.agents = []  # va pas marcher avec jax ?
